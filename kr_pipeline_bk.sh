...
# Copies Fastq files from batch directory to working directory
# Runs Trimmomatic on paired FASTQ files for quality trimming
# Runs Kraken2 on paired trimmed FASTQ files to assign taxonomoy
# Runs Bracken on K2 report file to re-estimate taxonomy
# Runs custom R script to generate sample-counts table from each sample's Braken estimate

# Required: Linux environment with Conda & R installed. 
...
# Make script executable: chmod +x kr_pipeline_bk.sh

# To run: 
# <path>/kr_pipeline_bk.sh <batch_no> <data_dir> <number_of_threads> <location_of_db>
...
source /home/chiranjit/miniconda3/etc/profile.d/conda.sh
